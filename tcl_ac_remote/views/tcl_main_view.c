...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every TclToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from TclToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)TclToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= TCL_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to TclExtra");
...
struct TclMainView {
    View* view;
    TclMainViewSendCallback send_callback;
    void* send_context;
    TclMainViewNavigateCallback extra_callback;
    void* extra_context;
    TclMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void tcl_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = tcl_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? TclModeCount : TclFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? tcl_ir_get_mode_name((TclMode)i) :
                             tcl_ir_get_fan_name((TclFan)i);
}

static void draw_picker(Canvas* canvas, TclMainViewModel* m) {
...
static void tcl_main_view_draw(Canvas* canvas, void* model) {
    TclMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = tcl_ir_get_fan_name(m->state->fan);
    if(m->state->mode == TclModeOff) {
        fan_value = "--";
    } else if(tcl_ir_mode_locks_fan(m->state->mode)) {
        fan_value = tcl_ir_get_fan_name(TclFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", tcl_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!tcl_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            TclToggle t = (TclToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, tcl_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            TclToggle t = (TclToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, tcl_ir_get_toggle_name(t), focused,
                tcl_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(TclMainView* view) {
    with_view_model(view->view, TclMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, TCL_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(TclMainView* view, TclToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        TclMainViewModel * m,
...
static void send_state_immediate(TclMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        TclMainViewModel * m,
...
static bool tcl_main_view_input(InputEvent* event, void* context) {
    TclMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            tcl_state_set_mode(m->state, (TclMode)m->picker_index);
                        } else {
                            tcl_state_set_fan(m->state, (TclFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        TclToggle t = (TclToggle)(f - SLOT_TOGGLE_0 + 1);
                        tcl_state_toggle(m->state, t);
...
TclMainView* tcl_main_view_alloc(void) {
    TclMainView* view = malloc(sizeof(TclMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(TclMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, tcl_main_view_draw);
    view_set_input_callback(view->view, tcl_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(tcl_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        TclMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = TclTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void tcl_main_view_set_state(TclMainView* view, TclState* state) {
    with_view_model(view->view, TclMainViewModel * m, { m->state = state; }, true);
}
...
void tcl_main_view_update_sending(TclMainView* view) {
    with_view_model(
        view->view, TclMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
