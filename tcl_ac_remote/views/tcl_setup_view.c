...
static uint8_t row_count(void) {
    return tcl_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void tcl_setup_view_draw(Canvas* canvas, void* model) {
    TclSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = tcl_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, tcl_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            tcl_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)tcl_ir_get_option_count());
...
static bool tcl_setup_view_input(InputEvent* event, void* context) {
    TclSetupView* view = context;
    bool consumed = false;
    ...
    with_view_model(
        ...
        TclSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                uint8_t opts = tcl_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
TclSetupView* tcl_setup_view_alloc(void) {
    TclSetupView* view = malloc(sizeof(TclSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(TclSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, tcl_setup_view_draw);
    view_set_input_callback(view->view, tcl_setup_view_input);
...
void tcl_setup_view_set_state(TclSetupView* view, TclState* state) {
    with_view_model(view->view, TclSetupViewModel * m, { m->state = state; }, true);
}
