...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every PanasonicToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from PanasonicToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)PanasonicToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= PANASONIC_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to PanasonicExtra");
...
typedef struct {
    ...
    PanasonicToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} PanasonicMainViewModel;

struct PanasonicMainView {
    View* view;
    PanasonicMainViewSendCallback send_callback;
    void* send_context;
    PanasonicMainViewNavigateCallback extra_callback;
    void* extra_context;
    PanasonicMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void panasonic_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = panasonic_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? PanasonicModeCount : PanasonicFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? panasonic_ir_get_mode_name((PanasonicMode)i) :
                             panasonic_ir_get_fan_name((PanasonicFan)i);
}

static void draw_picker(Canvas* canvas, PanasonicMainViewModel* m) {
...
static void panasonic_main_view_draw(Canvas* canvas, void* model) {
    PanasonicMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = panasonic_ir_get_fan_name(m->state->fan);
    if(m->state->mode == PanasonicModeOff) {
        fan_value = "--";
    } else if(panasonic_ir_mode_locks_fan(m->state->mode)) {
        fan_value = panasonic_ir_get_fan_name(PanasonicFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", panasonic_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!panasonic_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            PanasonicToggle t = (PanasonicToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, panasonic_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            PanasonicToggle t = (PanasonicToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, panasonic_ir_get_toggle_name(t), focused,
                panasonic_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(PanasonicMainView* view) {
    with_view_model(view->view, PanasonicMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, PANASONIC_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(PanasonicMainView* view, PanasonicToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        PanasonicMainViewModel * m,
        {
...
static void send_state_immediate(PanasonicMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        PanasonicMainViewModel * m,
...
static bool panasonic_main_view_input(InputEvent* event, void* context) {
    PanasonicMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            panasonic_state_set_mode(m->state, (PanasonicMode)m->picker_index);
                        } else {
                            panasonic_state_set_fan(m->state, (PanasonicFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        PanasonicToggle t = (PanasonicToggle)(f - SLOT_TOGGLE_0 + 1);
                        panasonic_state_toggle(m->state, t);
...
PanasonicMainView* panasonic_main_view_alloc(void) {
    PanasonicMainView* view = malloc(sizeof(PanasonicMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(PanasonicMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, panasonic_main_view_draw);
    view_set_input_callback(view->view, panasonic_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(panasonic_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        PanasonicMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = PanasonicTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void panasonic_main_view_set_state(PanasonicMainView* view, PanasonicState* state) {
    with_view_model(view->view, PanasonicMainViewModel * m, { m->state = state; }, true);
}
...
void panasonic_main_view_update_sending(PanasonicMainView* view) {
    with_view_model(
        view->view, PanasonicMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
