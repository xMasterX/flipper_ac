#include "panasonic_extra_view.h"
...
} PanasonicExtraViewModel;

struct PanasonicExtraView {
    View* view;
    PanasonicExtraViewSendCallback send_callback;
    void* send_context;
};

static void panasonic_extra_view_draw(Canvas* canvas, void* model) {
    PanasonicExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)PanasonicExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, panasonic_ir_get_extra_name((PanasonicExtra)i));
    ...
    char buf[PANASONIC_CODE_STR_LEN];
    ...
    PanasonicRequest req = panasonic_state_request(m->state);
    panasonic_ir_format_extra(&req, (PanasonicExtra)m->selected, buf, sizeof(buf));
...
static bool panasonic_extra_view_input(InputEvent* event, void* context) {
    PanasonicExtraView* view = context;
    bool consumed = false;
    bool send = false;
    PanasonicExtra to_send = (PanasonicExtra)0;

    with_view_model(
        view->view,
        PanasonicExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)PanasonicExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (PanasonicExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
PanasonicExtraView* panasonic_extra_view_alloc(void) {
    PanasonicExtraView* view = malloc(sizeof(PanasonicExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(PanasonicExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, panasonic_extra_view_draw);
    view_set_input_callback(view->view, panasonic_extra_view_input);
...
void panasonic_extra_view_set_state(PanasonicExtraView* view, PanasonicState* state) {
    with_view_model(view->view, PanasonicExtraViewModel * m, { m->state = state; }, true);
}

void panasonic_extra_view_set_last_sent(PanasonicExtraView* view, const char* last_sent) {
    with_view_model(view->view, PanasonicExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
