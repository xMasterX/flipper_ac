#include "kelvinator_main_view.h"
...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every KelvinatorToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from KelvinatorToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)KelvinatorToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= KELVINATOR_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to KelvinatorExtra");
...
typedef struct {
    ...
    KelvinatorToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} KelvinatorMainViewModel;

struct KelvinatorMainView {
    View* view;
    KelvinatorMainViewSendCallback send_callback;
    void* send_context;
    KelvinatorMainViewNavigateCallback extra_callback;
    void* extra_context;
    KelvinatorMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void kelvinator_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = kelvinator_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? KelvinatorModeCount : KelvinatorFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? kelvinator_ir_get_mode_name((KelvinatorMode)i) :
                             kelvinator_ir_get_fan_name((KelvinatorFan)i);
}

static void draw_picker(Canvas* canvas, KelvinatorMainViewModel* m) {
...
static void kelvinator_main_view_draw(Canvas* canvas, void* model) {
    KelvinatorMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = kelvinator_ir_get_fan_name(m->state->fan);
    if(m->state->mode == KelvinatorModeOff) {
        fan_value = "--";
    } else if(kelvinator_ir_mode_locks_fan(m->state->mode)) {
        fan_value = kelvinator_ir_get_fan_name(KelvinatorFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", kelvinator_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!kelvinator_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            KelvinatorToggle t = (KelvinatorToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, kelvinator_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            KelvinatorToggle t = (KelvinatorToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, kelvinator_ir_get_toggle_name(t), focused,
                kelvinator_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(KelvinatorMainView* view) {
    with_view_model(view->view, KelvinatorMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, KELVINATOR_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(KelvinatorMainView* view, KelvinatorToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        KelvinatorMainViewModel * m,
        {
...
static void send_state_immediate(KelvinatorMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        KelvinatorMainViewModel * m,
...
static bool kelvinator_main_view_input(InputEvent* event, void* context) {
    KelvinatorMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            kelvinator_state_set_mode(m->state, (KelvinatorMode)m->picker_index);
                        } else {
                            kelvinator_state_set_fan(m->state, (KelvinatorFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        KelvinatorToggle t = (KelvinatorToggle)(f - SLOT_TOGGLE_0 + 1);
                        kelvinator_state_toggle(m->state, t);
...
KelvinatorMainView* kelvinator_main_view_alloc(void) {
    KelvinatorMainView* view = malloc(sizeof(KelvinatorMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelvinatorMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelvinator_main_view_draw);
    view_set_input_callback(view->view, kelvinator_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(kelvinator_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        KelvinatorMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = KelvinatorTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void kelvinator_main_view_set_state(KelvinatorMainView* view, KelvinatorState* state) {
    with_view_model(view->view, KelvinatorMainViewModel * m, { m->state = state; }, true);
}
...
void kelvinator_main_view_update_sending(KelvinatorMainView* view) {
    with_view_model(
        view->view, KelvinatorMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
