#include "kelvinator_extra_view.h"
...
} KelvinatorExtraViewModel;

struct KelvinatorExtraView {
    View* view;
    KelvinatorExtraViewSendCallback send_callback;
    void* send_context;
};

static void kelvinator_extra_view_draw(Canvas* canvas, void* model) {
    KelvinatorExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)KelvinatorExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, kelvinator_ir_get_extra_name((KelvinatorExtra)i));
    ...
    char buf[KELVINATOR_CODE_STR_LEN];
    ...
    KelvinatorRequest req = kelvinator_state_request(m->state);
    kelvinator_ir_format_extra(&req, (KelvinatorExtra)m->selected, buf, sizeof(buf));
...
static bool kelvinator_extra_view_input(InputEvent* event, void* context) {
    KelvinatorExtraView* view = context;
    bool consumed = false;
    bool send = false;
    KelvinatorExtra to_send = (KelvinatorExtra)0;

    with_view_model(
        view->view,
        KelvinatorExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)KelvinatorExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (KelvinatorExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
KelvinatorExtraView* kelvinator_extra_view_alloc(void) {
    KelvinatorExtraView* view = malloc(sizeof(KelvinatorExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelvinatorExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelvinator_extra_view_draw);
    view_set_input_callback(view->view, kelvinator_extra_view_input);
...
void kelvinator_extra_view_set_state(KelvinatorExtraView* view, KelvinatorState* state) {
    with_view_model(view->view, KelvinatorExtraViewModel * m, { m->state = state; }, true);
}

void kelvinator_extra_view_set_last_sent(KelvinatorExtraView* view, const char* last_sent) {
    with_view_model(view->view, KelvinatorExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
