...
static KelvinatorApp* kelvinator_app_alloc(void) {
    ...
    view_dispatcher_set_navigation_event_callback(app->view_dispatcher, kelvinator_app_back_callback);
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, KelvinatorViewExtra, kelvinator_extra_view_get_view(app->extra_view));

    // Setup view
    app->setup_view = kelvinator_setup_view_alloc();
    kelvinator_setup_view_set_state(app->setup_view, app->state);
    view_dispatcher_add_view(
        app->view_dispatcher, KelvinatorViewSetup, kelvinator_setup_view_get_view(app->setup_view));
    ...
    app->ir_tx_stop_timer = furi_timer_alloc(kelvinator_app_tx_stop_callback, FuriTimerTypeOnce, app);
    app->is_sending = false;
