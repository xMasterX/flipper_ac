...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every GreeToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from GreeToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)GreeToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= GREE_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to GreeExtra");
...
struct GreeMainView {
    View* view;
    GreeMainViewSendCallback send_callback;
    void* send_context;
    GreeMainViewNavigateCallback extra_callback;
    void* extra_context;
    GreeMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void gree_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = gree_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? GreeModeCount : GreeFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? gree_ir_get_mode_name((GreeMode)i) :
                             gree_ir_get_fan_name((GreeFan)i);
}

static void draw_picker(Canvas* canvas, GreeMainViewModel* m) {
...
static void gree_main_view_draw(Canvas* canvas, void* model) {
    GreeMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = gree_ir_get_fan_name(m->state->fan);
    if(m->state->mode == GreeModeOff) {
        fan_value = "--";
    } else if(gree_ir_mode_locks_fan(m->state->mode)) {
        fan_value = gree_ir_get_fan_name(GreeFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", gree_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!gree_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            GreeToggle t = (GreeToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, gree_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            GreeToggle t = (GreeToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, gree_ir_get_toggle_name(t), focused,
                gree_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(GreeMainView* view) {
    with_view_model(view->view, GreeMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, GREE_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(GreeMainView* view, GreeToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        GreeMainViewModel * m,
...
static void send_state_immediate(GreeMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        GreeMainViewModel * m,
...
static bool gree_main_view_input(InputEvent* event, void* context) {
    GreeMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            gree_state_set_mode(m->state, (GreeMode)m->picker_index);
                        } else {
                            gree_state_set_fan(m->state, (GreeFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        GreeToggle t = (GreeToggle)(f - SLOT_TOGGLE_0 + 1);
                        gree_state_toggle(m->state, t);
...
GreeMainView* gree_main_view_alloc(void) {
    GreeMainView* view = malloc(sizeof(GreeMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GreeMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, gree_main_view_draw);
    view_set_input_callback(view->view, gree_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(gree_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        GreeMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = GreeTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void gree_main_view_set_state(GreeMainView* view, GreeState* state) {
    with_view_model(view->view, GreeMainViewModel * m, { m->state = state; }, true);
}
...
void gree_main_view_update_sending(GreeMainView* view) {
    with_view_model(
        view->view, GreeMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
