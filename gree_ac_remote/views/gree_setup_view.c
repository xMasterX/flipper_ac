...
struct GreeSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return gree_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void gree_setup_view_draw(Canvas* canvas, void* model) {
    GreeSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = gree_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, gree_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            gree_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)gree_ir_get_option_count());
...
static bool gree_setup_view_input(InputEvent* event, void* context) {
    GreeSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        GreeSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = gree_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
GreeSetupView* gree_setup_view_alloc(void) {
    GreeSetupView* view = malloc(sizeof(GreeSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GreeSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, gree_setup_view_draw);
    view_set_input_callback(view->view, gree_setup_view_input);
...
void gree_setup_view_set_state(GreeSetupView* view, GreeState* state) {
    with_view_model(view->view, GreeSetupViewModel * m, { m->state = state; }, true);
}
