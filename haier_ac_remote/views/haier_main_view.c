...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every HaierToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from HaierToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)HaierToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= HAIER_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to HaierExtra");
...
struct HaierMainView {
    View* view;
    HaierMainViewSendCallback send_callback;
    void* send_context;
    HaierMainViewNavigateCallback extra_callback;
    void* extra_context;
    HaierMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void haier_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = haier_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? HaierModeCount : HaierFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? haier_ir_get_mode_name((HaierMode)i) :
                             haier_ir_get_fan_name((HaierFan)i);
}

static void draw_picker(Canvas* canvas, HaierMainViewModel* m) {
...
static void haier_main_view_draw(Canvas* canvas, void* model) {
    HaierMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = haier_ir_get_fan_name(m->state->fan);
    if(m->state->mode == HaierModeOff) {
        fan_value = "--";
    } else if(haier_ir_mode_locks_fan(m->state->mode)) {
        fan_value = haier_ir_get_fan_name(HaierFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", haier_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!haier_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            HaierToggle t = (HaierToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, haier_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            HaierToggle t = (HaierToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, haier_ir_get_toggle_name(t), focused,
                haier_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(HaierMainView* view) {
    with_view_model(view->view, HaierMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, HAIER_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(HaierMainView* view, HaierToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        HaierMainViewModel * m,
        {
...
static void send_state_immediate(HaierMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        HaierMainViewModel * m,
...
static bool haier_main_view_input(InputEvent* event, void* context) {
    HaierMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            haier_state_set_mode(m->state, (HaierMode)m->picker_index);
                        } else {
                            haier_state_set_fan(m->state, (HaierFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        HaierToggle t = (HaierToggle)(f - SLOT_TOGGLE_0 + 1);
                        haier_state_toggle(m->state, t);
...
HaierMainView* haier_main_view_alloc(void) {
    HaierMainView* view = malloc(sizeof(HaierMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(HaierMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, haier_main_view_draw);
    view_set_input_callback(view->view, haier_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(haier_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        HaierMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = HaierTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void haier_main_view_set_state(HaierMainView* view, HaierState* state) {
    with_view_model(view->view, HaierMainViewModel * m, { m->state = state; }, true);
}
...
void haier_main_view_update_sending(HaierMainView* view) {
    with_view_model(
        view->view, HaierMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
