...
struct HaierSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return haier_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void haier_setup_view_draw(Canvas* canvas, void* model) {
    HaierSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = haier_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, haier_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            haier_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)haier_ir_get_option_count());
...
static bool haier_setup_view_input(InputEvent* event, void* context) {
    HaierSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        HaierSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = haier_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
HaierSetupView* haier_setup_view_alloc(void) {
    HaierSetupView* view = malloc(sizeof(HaierSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(HaierSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, haier_setup_view_draw);
    view_set_input_callback(view->view, haier_setup_view_input);
...
void haier_setup_view_set_state(HaierSetupView* view, HaierState* state) {
    with_view_model(view->view, HaierSetupViewModel * m, { m->state = state; }, true);
}
