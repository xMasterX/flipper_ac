...
static void build_state_hsu(const HaierRequest* req, bool power, bool swing_v, uint8_t* st) {
    ...
    memset(st, 0, HSU_LEN);
    st[0] = HSU_PREFIX;
    st[1] = (uint8_t)((power ? HSU_CMD_ON : HSU_CMD_OFF) |
                      (((temp - HAIER_TEMP_MIN) & 0x0F) << 4));
...
bool haier_ir_encode_extra(
    ...
    static const uint8_t VANE[4] = {
        H_SWINGV_TOP, H_SWINGV_MIDDLE, H_SWINGV_BOTTOM, H_SWINGV_DOWN};
    ...
    return encode_for_model(
        req, true, swing_v_for(req), h, H_BTN_SWINGH, timings, timings_count);
}
