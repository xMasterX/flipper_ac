...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every KelonToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from KelonToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)KelonToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= KELON_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to KelonExtra");
...
struct KelonMainView {
    View* view;
    KelonMainViewSendCallback send_callback;
    void* send_context;
    KelonMainViewNavigateCallback extra_callback;
    void* extra_context;
    KelonMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void kelon_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = kelon_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? KelonModeCount : KelonFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? kelon_ir_get_mode_name((KelonMode)i) :
                             kelon_ir_get_fan_name((KelonFan)i);
}

static void draw_picker(Canvas* canvas, KelonMainViewModel* m) {
...
static void kelon_main_view_draw(Canvas* canvas, void* model) {
    KelonMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = kelon_ir_get_fan_name(m->state->fan);
    if(m->state->mode == KelonModeOff) {
        fan_value = "--";
    } else if(kelon_ir_mode_locks_fan(m->state->mode)) {
        fan_value = kelon_ir_get_fan_name(KelonFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", kelon_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!kelon_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            KelonToggle t = (KelonToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, kelon_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            KelonToggle t = (KelonToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, kelon_ir_get_toggle_name(t), focused,
                kelon_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(KelonMainView* view) {
    with_view_model(view->view, KelonMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, KELON_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(KelonMainView* view, KelonToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        KelonMainViewModel * m,
        {
...
static void send_state_immediate(KelonMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        KelonMainViewModel * m,
...
static bool kelon_main_view_input(InputEvent* event, void* context) {
    KelonMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            kelon_state_set_mode(m->state, (KelonMode)m->picker_index);
                        } else {
                            kelon_state_set_fan(m->state, (KelonFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        KelonToggle t = (KelonToggle)(f - SLOT_TOGGLE_0 + 1);
                        kelon_state_toggle(m->state, t);
...
KelonMainView* kelon_main_view_alloc(void) {
    KelonMainView* view = malloc(sizeof(KelonMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelonMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelon_main_view_draw);
    view_set_input_callback(view->view, kelon_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(kelon_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        KelonMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = KelonTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void kelon_main_view_set_state(KelonMainView* view, KelonState* state) {
    with_view_model(view->view, KelonMainViewModel * m, { m->state = state; }, true);
}
...
void kelon_main_view_update_sending(KelonMainView* view) {
    with_view_model(
        view->view, KelonMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
