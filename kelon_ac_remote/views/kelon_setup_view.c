...
struct KelonSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return kelon_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void kelon_setup_view_draw(Canvas* canvas, void* model) {
    KelonSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = kelon_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, kelon_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            kelon_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)kelon_ir_get_option_count());
...
static bool kelon_setup_view_input(InputEvent* event, void* context) {
    KelonSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        KelonSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = kelon_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
KelonSetupView* kelon_setup_view_alloc(void) {
    KelonSetupView* view = malloc(sizeof(KelonSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(KelonSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, kelon_setup_view_draw);
    view_set_input_callback(view->view, kelon_setup_view_input);
...
void kelon_setup_view_set_state(KelonSetupView* view, KelonState* state) {
    with_view_model(view->view, KelonSetupViewModel * m, { m->state = state; }, true);
}
