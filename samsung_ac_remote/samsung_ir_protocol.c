...
static void build_state(
    ...
    uint8_t fan_code = (mode_code == S_AUTO) ? S_FAN_AUTO2 :
                                               FAN_CODES[req->fan % SamsungFanCount];
...
static bool encode_sections(const uint8_t* st, uint8_t len, uint32_t* timings, size_t* count) {
    ...
    for(uint8_t s = 0; s < sections; s++) {
        ...
        ir_bytes_lsb(&b, st + (size_t)s * SECTION_LEN, SECTION_LEN, BIT_MARK, ONE_SPACE, ZERO_SPACE);
