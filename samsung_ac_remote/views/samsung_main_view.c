...
static void samsung_main_view_draw(Canvas* canvas, void* model) {
    ...
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", samsung_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            SamsungToggle t = (SamsungToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, samsung_ir_get_toggle_name(t), focused,
...
void samsung_main_view_update_sending(SamsungMainView* view) {
    with_view_model(
        view->view, SamsungMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
