...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every DaikinToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from DaikinToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)DaikinToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= DAIKIN_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to DaikinExtra");
...
struct DaikinMainView {
    View* view;
    DaikinMainViewSendCallback send_callback;
    void* send_context;
    DaikinMainViewNavigateCallback extra_callback;
    void* extra_context;
    DaikinMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void daikin_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = daikin_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? DaikinModeCount : DaikinFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? daikin_ir_get_mode_name((DaikinMode)i) :
                             daikin_ir_get_fan_name((DaikinFan)i);
}

static void draw_picker(Canvas* canvas, DaikinMainViewModel* m) {
...
static void daikin_main_view_draw(Canvas* canvas, void* model) {
    DaikinMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = daikin_ir_get_fan_name(m->state->fan);
    if(m->state->mode == DaikinModeOff) {
        fan_value = "--";
    } else if(daikin_ir_mode_locks_fan(m->state->mode)) {
        fan_value = daikin_ir_get_fan_name(DaikinFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", daikin_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!daikin_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            DaikinToggle t = (DaikinToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, daikin_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            DaikinToggle t = (DaikinToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, daikin_ir_get_toggle_name(t), focused,
                daikin_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(DaikinMainView* view) {
    with_view_model(view->view, DaikinMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, DAIKIN_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(DaikinMainView* view, DaikinToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        DaikinMainViewModel * m,
        {
...
static void send_state_immediate(DaikinMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        DaikinMainViewModel * m,
...
static bool daikin_main_view_input(InputEvent* event, void* context) {
    DaikinMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            daikin_state_set_mode(m->state, (DaikinMode)m->picker_index);
                        } else {
                            daikin_state_set_fan(m->state, (DaikinFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        DaikinToggle t = (DaikinToggle)(f - SLOT_TOGGLE_0 + 1);
                        daikin_state_toggle(m->state, t);
...
DaikinMainView* daikin_main_view_alloc(void) {
    DaikinMainView* view = malloc(sizeof(DaikinMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(DaikinMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, daikin_main_view_draw);
    view_set_input_callback(view->view, daikin_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(daikin_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        DaikinMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = DaikinTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void daikin_main_view_set_state(DaikinMainView* view, DaikinState* state) {
    with_view_model(view->view, DaikinMainViewModel * m, { m->state = state; }, true);
}
...
void daikin_main_view_update_sending(DaikinMainView* view) {
    with_view_model(
        view->view, DaikinMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
