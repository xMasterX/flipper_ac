...
#define SUM_BYTES(dst, src, n)                        \
    do {                                              \
        uint8_t s_ = 0;                               \
        for(uint8_t i_ = 0; i_ < (n); i_++) s_ = (uint8_t)(s_ + (src)[i_]); \
        (dst) = s_;                                   \
    } while(0)
...
static void build_2(const DaikinRequest* req, bool power, bool swing_h, uint8_t* st) {
    static const uint8_t base[D2_LEN] = {
        0x11, 0xDA, 0x27, 0x00, 0x01, 0x00, 0xC0, 0x70, 0x08, 0x0C, 0x80, 0x04,
        0xB0, 0x16, 0x24, 0x00, 0x00, 0xBE, 0xD0, 0x00, 0x11, 0xDA, 0x27, 0x00,
        0x00, 0x08, 0x00, 0x00, 0xA0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xC1,
        0x80, 0x60, 0x00};
...
static void build_216(const DaikinRequest* req, bool power, bool swing_h, uint8_t* st) {
    static const uint8_t base[D216_LEN] = {
        0x11, 0xDA, 0x27, 0xF0, 0x00, 0x00, 0x00, 0x00, 0x11, 0xDA, 0x27, 0x00,
...
static void build_160(const DaikinRequest* req, bool power, uint8_t* st) {
    static const uint8_t base[D160_LEN] = {
        0x11, 0xDA, 0x27, 0xF0, 0x0D, 0x00, 0x00, 0x11, 0xDA, 0x27, 0x00, 0xD3,
        0x30, 0x11, 0x00, 0x00, 0x1E, 0x0A, 0x08, 0x00};
    ...
    st[13] = (uint8_t)((st[13] & 0x0F) |
                       ((swing_v ? D160_SWINGV_AUTO : D160_SWINGV_OFF) << 4));
...
static void build_176(const DaikinRequest* req, bool power, bool swing_h, uint8_t* st) {
    static const uint8_t base[D176_LEN] = {
        0x11, 0xDA, 0x17, 0x18, 0x04, 0x00, 0x00, 0x11, 0xDA, 0x17, 0x18, 0x00,
        0x73, 0x00, 0x20, 0x00, 0x00, 0x00, 0x16, 0x00, 0x20, 0x00};
...
static void build_152(const DaikinRequest* req, bool power, uint8_t* st) {
    static const uint8_t base[D152_LEN] = {
        0x11, 0xDA, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void build_128(const DaikinRequest* req, bool power, uint8_t* st) {
    static const uint8_t base[D128_LEN] = {
        0x16, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0xA1, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00};
    ...
    if(mode_code != D128_AUTO) {
        if((req->toggle_bits >> DaikinTogglePowerful) & 1) fan = D128_FAN_POWERFUL;
        else if((req->toggle_bits >> DaikinToggleQuiet) & 1) fan = D128_FAN_QUIET;
    }
    ...
    st[7] = (uint8_t)((st[7] & 0x0F) | ((sum_nibbles(st, D128_SEC - 1, st[7] & 0x0F) & 0x0F) << 4));
...
static uint64_t build_64(const DaikinRequest* req, bool power) {
    ...
    if((req->toggle_bits >> DaikinTogglePowerful) & 1) fan = D64_FAN_TURBO;
    else if((req->toggle_bits >> DaikinToggleQuiet) & 1) fan = D64_FAN_QUIET;
...
static void daikin_checksums(uint8_t* st) {
    uint8_t s = 0;
    for(uint8_t i = 0; i < SECTION1 - 1; i++) s += st[i];
    st[SECTION1 - 1] = s;

    s = 0;
    for(uint8_t i = SECTION1; i < SECTION1 + SECTION2 - 1; i++) s += st[i];
    st[SECTION1 + SECTION2 - 1] = s;

    s = 0;
    for(uint8_t i = SECTION1 + SECTION2; i < STATE_LEN - 1; i++) s += st[i];
    st[STATE_LEN - 1] = s;
}
...
static void build_state(
    ...
    st[24] = (uint8_t)(
        (((tb >> DaikinToggleSwing) & 1) ? D_SWING_ON : D_SWING_OFF) | ((fan_field & 0x0F) << 4));
...
static bool encode_for_model(
    ...
    case DaikinModel216:
        ...
        return encode_two_sections(
            st, D216_LEN, D216_SEC1, D216_HDR_MARK, D216_HDR_SPACE, D216_BIT_MARK,
            D216_ONE_SPACE, D216_ZERO_SPACE, D216_GAP, t, n);
    case DaikinModel160:
        build_160(req, power, st);
        return encode_two_sections(
            st, D160_LEN, D160_SEC1, D160_HDR_MARK, D160_HDR_SPACE, D160_BIT_MARK,
            D160_ONE_SPACE, D160_ZERO_SPACE, D160_GAP, t, n);
    case DaikinModel176:
        build_176(req, power, swing_h, st);
        return encode_two_sections(
            st, D176_LEN, D176_SEC1, D176_HDR_MARK, D176_HDR_SPACE, D176_BIT_MARK,
            D176_ONE_SPACE, D176_ZERO_SPACE, D176_GAP, t, n);
    case DaikinModel152:
