...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every ToshibaToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from ToshibaToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)ToshibaToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= TOSHIBA_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to ToshibaExtra");
...
typedef struct {
    ...
    ToshibaToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} ToshibaMainViewModel;

struct ToshibaMainView {
    View* view;
    ToshibaMainViewSendCallback send_callback;
    void* send_context;
    ToshibaMainViewNavigateCallback extra_callback;
    void* extra_context;
    ToshibaMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void toshiba_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = toshiba_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? ToshibaModeCount : ToshibaFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? toshiba_ir_get_mode_name((ToshibaMode)i) :
                             toshiba_ir_get_fan_name((ToshibaFan)i);
}

static void draw_picker(Canvas* canvas, ToshibaMainViewModel* m) {
...
static void toshiba_main_view_draw(Canvas* canvas, void* model) {
    ToshibaMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = toshiba_ir_get_fan_name(m->state->fan);
    if(m->state->mode == ToshibaModeOff) {
        fan_value = "--";
    } else if(toshiba_ir_mode_locks_fan(m->state->mode)) {
        fan_value = toshiba_ir_get_fan_name(ToshibaFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", toshiba_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!toshiba_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            ToshibaToggle t = (ToshibaToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, toshiba_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            ToshibaToggle t = (ToshibaToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, toshiba_ir_get_toggle_name(t), focused,
                toshiba_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(ToshibaMainView* view) {
    with_view_model(view->view, ToshibaMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, TOSHIBA_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(ToshibaMainView* view, ToshibaToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        ToshibaMainViewModel * m,
        {
...
static void send_state_immediate(ToshibaMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        ToshibaMainViewModel * m,
...
static bool toshiba_main_view_input(InputEvent* event, void* context) {
    ToshibaMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            toshiba_state_set_mode(m->state, (ToshibaMode)m->picker_index);
                        } else {
                            toshiba_state_set_fan(m->state, (ToshibaFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        ToshibaToggle t = (ToshibaToggle)(f - SLOT_TOGGLE_0 + 1);
                        toshiba_state_toggle(m->state, t);
...
ToshibaMainView* toshiba_main_view_alloc(void) {
    ToshibaMainView* view = malloc(sizeof(ToshibaMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(ToshibaMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, toshiba_main_view_draw);
    view_set_input_callback(view->view, toshiba_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(toshiba_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        ToshibaMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = ToshibaTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void toshiba_main_view_set_state(ToshibaMainView* view, ToshibaState* state) {
    with_view_model(view->view, ToshibaMainViewModel * m, { m->state = state; }, true);
}
...
void toshiba_main_view_update_sending(ToshibaMainView* view) {
    with_view_model(
        view->view, ToshibaMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
