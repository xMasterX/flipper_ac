...
struct MideaSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return midea_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void midea_setup_view_draw(Canvas* canvas, void* model) {
    MideaSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = midea_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, midea_ir_get_option_label());
        ...
        snprintf(
            ...
            (unsigned)midea_ir_get_option_count());
...
static bool midea_setup_view_input(InputEvent* event, void* context) {
    MideaSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        MideaSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = midea_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
MideaSetupView* midea_setup_view_alloc(void) {
    MideaSetupView* view = malloc(sizeof(MideaSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MideaSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, midea_setup_view_draw);
    view_set_input_callback(view->view, midea_setup_view_input);
...
void midea_setup_view_set_state(MideaSetupView* view, MideaState* state) {
    with_view_model(view->view, MideaSetupViewModel * m, { m->state = state; }, true);
}
