...
static void draw_header(Canvas* canvas, const DetectorModel* m) {
    ...
    canvas_draw_str_aligned(canvas, SCREEN_W / 2, TITLE_BASE, AlignCenter, AlignBottom, "AC Detector");
...
static bool detector_view_input(InputEvent* event, void* context) {
    ...
    case InputKeyRight: {
        ...
        with_view_model(
            ...
            {
                if(m->has_result) {
                    m->page = (uint8_t)((m->page + (event->key == InputKeyRight ? 1 : PageCount - 1)) % PageCount);
                    m->scroll = 0;
                    changed = true;
                }
            },
