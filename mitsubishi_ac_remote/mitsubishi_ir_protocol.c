...
static const char* const MODEL_NAMES[MitsubishiModelCount] =
    {"144-bit", "112-bit", "136-bit"};
...
static void build_state(
    ...
    uint8_t fan = tog(req, MitsubishiToggleQuiet) ? F_SILENT : FAN_CODES[req->fan % MitsubishiFanCount];
...
static void build_state_112(
    ...
    st[8] = (uint8_t)((FAN_CODES_112[req->fan % MitsubishiFanCount] & 0x07) |
                      ((vane & 0x07) << 3));
...
static void build_state_136(
    const MitsubishiRequest* req,
    bool power,
    uint8_t vane,
    uint8_t* st) {
    static const uint8_t base[S136_LEN] = {
        0x23, 0xCB, 0x26, 0x21, 0x00, 0x40, 0xC2, 0xC7, 0x04, 0x00, 0x00,
...
bool mitsubishi_ir_encode_extra(
    ...
    static const uint8_t WIDES[] =
        {W_LEFT_MAX, W_LEFT, W_MIDDLE, W_RIGHT, W_RIGHT_MAX, W_WIDE};
...
void mitsubishi_ir_format_toggle(
    ...
    snprintf(
        ...
        "%s %s",
        MODEL_NAMES[model],
        toggle == MitsubishiTogglePowerOff ? "off" : "on");
}
