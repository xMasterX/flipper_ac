#include "mitsubishi_extra_view.h"
...
} MitsubishiExtraViewModel;

struct MitsubishiExtraView {
    View* view;
    MitsubishiExtraViewSendCallback send_callback;
    void* send_context;
};

static void mitsubishi_extra_view_draw(Canvas* canvas, void* model) {
    MitsubishiExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)MitsubishiExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, mitsubishi_ir_get_extra_name((MitsubishiExtra)i));
    ...
    char buf[MITSUBISHI_CODE_STR_LEN];
    ...
    MitsubishiRequest req = mitsubishi_state_request(m->state);
    mitsubishi_ir_format_extra(&req, (MitsubishiExtra)m->selected, buf, sizeof(buf));
...
static bool mitsubishi_extra_view_input(InputEvent* event, void* context) {
    MitsubishiExtraView* view = context;
    bool consumed = false;
    bool send = false;
    MitsubishiExtra to_send = (MitsubishiExtra)0;

    with_view_model(
        view->view,
        MitsubishiExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)MitsubishiExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (MitsubishiExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
MitsubishiExtraView* mitsubishi_extra_view_alloc(void) {
    MitsubishiExtraView* view = malloc(sizeof(MitsubishiExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MitsubishiExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, mitsubishi_extra_view_draw);
    view_set_input_callback(view->view, mitsubishi_extra_view_input);
...
void mitsubishi_extra_view_set_state(MitsubishiExtraView* view, MitsubishiState* state) {
    with_view_model(view->view, MitsubishiExtraViewModel * m, { m->state = state; }, true);
}

void mitsubishi_extra_view_set_last_sent(MitsubishiExtraView* view, const char* last_sent) {
    with_view_model(view->view, MitsubishiExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
