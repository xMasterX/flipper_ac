#include "mitsubishi_main_view.h"
...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every MitsubishiToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from MitsubishiToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)MitsubishiToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= MITSUBISHI_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to MitsubishiExtra");
...
typedef struct {
    ...
    MitsubishiToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} MitsubishiMainViewModel;

struct MitsubishiMainView {
    View* view;
    MitsubishiMainViewSendCallback send_callback;
    void* send_context;
    MitsubishiMainViewNavigateCallback extra_callback;
    void* extra_context;
    MitsubishiMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void mitsubishi_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = mitsubishi_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? MitsubishiModeCount : MitsubishiFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? mitsubishi_ir_get_mode_name((MitsubishiMode)i) :
                             mitsubishi_ir_get_fan_name((MitsubishiFan)i);
}

static void draw_picker(Canvas* canvas, MitsubishiMainViewModel* m) {
...
static void mitsubishi_main_view_draw(Canvas* canvas, void* model) {
    MitsubishiMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = mitsubishi_ir_get_fan_name(m->state->fan);
    if(m->state->mode == MitsubishiModeOff) {
        fan_value = "--";
    } else if(mitsubishi_ir_mode_locks_fan(m->state->mode)) {
        fan_value = mitsubishi_ir_get_fan_name(MitsubishiFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", mitsubishi_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!mitsubishi_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            MitsubishiToggle t = (MitsubishiToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, mitsubishi_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            MitsubishiToggle t = (MitsubishiToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, mitsubishi_ir_get_toggle_name(t), focused,
                mitsubishi_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(MitsubishiMainView* view) {
    with_view_model(view->view, MitsubishiMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, MITSUBISHI_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(MitsubishiMainView* view, MitsubishiToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        MitsubishiMainViewModel * m,
        {
...
static void send_state_immediate(MitsubishiMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        MitsubishiMainViewModel * m,
...
static bool mitsubishi_main_view_input(InputEvent* event, void* context) {
    MitsubishiMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            mitsubishi_state_set_mode(m->state, (MitsubishiMode)m->picker_index);
                        } else {
                            mitsubishi_state_set_fan(m->state, (MitsubishiFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        MitsubishiToggle t = (MitsubishiToggle)(f - SLOT_TOGGLE_0 + 1);
                        mitsubishi_state_toggle(m->state, t);
...
MitsubishiMainView* mitsubishi_main_view_alloc(void) {
    MitsubishiMainView* view = malloc(sizeof(MitsubishiMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MitsubishiMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, mitsubishi_main_view_draw);
    view_set_input_callback(view->view, mitsubishi_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(mitsubishi_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        MitsubishiMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = MitsubishiTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void mitsubishi_main_view_set_state(MitsubishiMainView* view, MitsubishiState* state) {
    with_view_model(view->view, MitsubishiMainViewModel * m, { m->state = state; }, true);
}
...
void mitsubishi_main_view_update_sending(MitsubishiMainView* view) {
    with_view_model(
        view->view, MitsubishiMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
