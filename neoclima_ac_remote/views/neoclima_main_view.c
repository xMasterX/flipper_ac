...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every NeoclimaToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from NeoclimaToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)NeoclimaToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= NEOCLIMA_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to NeoclimaExtra");
...
typedef struct {
    ...
    NeoclimaToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} NeoclimaMainViewModel;

struct NeoclimaMainView {
    View* view;
    NeoclimaMainViewSendCallback send_callback;
    void* send_context;
    NeoclimaMainViewNavigateCallback extra_callback;
    void* extra_context;
    NeoclimaMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void neoclima_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = neoclima_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? NeoclimaModeCount : NeoclimaFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? neoclima_ir_get_mode_name((NeoclimaMode)i) :
                             neoclima_ir_get_fan_name((NeoclimaFan)i);
}

static void draw_picker(Canvas* canvas, NeoclimaMainViewModel* m) {
...
static void neoclima_main_view_draw(Canvas* canvas, void* model) {
    NeoclimaMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = neoclima_ir_get_fan_name(m->state->fan);
    if(m->state->mode == NeoclimaModeOff) {
        fan_value = "--";
    } else if(neoclima_ir_mode_locks_fan(m->state->mode)) {
        fan_value = neoclima_ir_get_fan_name(NeoclimaFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", neoclima_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!neoclima_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            NeoclimaToggle t = (NeoclimaToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, neoclima_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            NeoclimaToggle t = (NeoclimaToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, neoclima_ir_get_toggle_name(t), focused,
                neoclima_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(NeoclimaMainView* view) {
    with_view_model(view->view, NeoclimaMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, NEOCLIMA_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(NeoclimaMainView* view, NeoclimaToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        NeoclimaMainViewModel * m,
        {
...
static void send_state_immediate(NeoclimaMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        NeoclimaMainViewModel * m,
...
static bool neoclima_main_view_input(InputEvent* event, void* context) {
    NeoclimaMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            neoclima_state_set_mode(m->state, (NeoclimaMode)m->picker_index);
                        } else {
                            neoclima_state_set_fan(m->state, (NeoclimaFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        NeoclimaToggle t = (NeoclimaToggle)(f - SLOT_TOGGLE_0 + 1);
                        neoclima_state_toggle(m->state, t);
...
NeoclimaMainView* neoclima_main_view_alloc(void) {
    NeoclimaMainView* view = malloc(sizeof(NeoclimaMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(NeoclimaMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, neoclima_main_view_draw);
    view_set_input_callback(view->view, neoclima_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(neoclima_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        NeoclimaMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = NeoclimaTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void neoclima_main_view_set_state(NeoclimaMainView* view, NeoclimaState* state) {
    with_view_model(view->view, NeoclimaMainViewModel * m, { m->state = state; }, true);
}
...
void neoclima_main_view_update_sending(NeoclimaMainView* view) {
    with_view_model(
        view->view, NeoclimaMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
