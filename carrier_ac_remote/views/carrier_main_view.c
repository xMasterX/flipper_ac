...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every CarrierToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from CarrierToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)CarrierToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= CARRIER_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to CarrierExtra");
...
typedef struct {
    ...
    CarrierToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} CarrierMainViewModel;

struct CarrierMainView {
    View* view;
    CarrierMainViewSendCallback send_callback;
    void* send_context;
    CarrierMainViewNavigateCallback extra_callback;
    void* extra_context;
    CarrierMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void carrier_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = carrier_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? CarrierModeCount : CarrierFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? carrier_ir_get_mode_name((CarrierMode)i) :
                             carrier_ir_get_fan_name((CarrierFan)i);
}

static void draw_picker(Canvas* canvas, CarrierMainViewModel* m) {
...
static void carrier_main_view_draw(Canvas* canvas, void* model) {
    CarrierMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = carrier_ir_get_fan_name(m->state->fan);
    if(m->state->mode == CarrierModeOff) {
        fan_value = "--";
    } else if(carrier_ir_mode_locks_fan(m->state->mode)) {
        fan_value = carrier_ir_get_fan_name(CarrierFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", carrier_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!carrier_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            CarrierToggle t = (CarrierToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, carrier_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            CarrierToggle t = (CarrierToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, carrier_ir_get_toggle_name(t), focused,
                carrier_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(CarrierMainView* view) {
    with_view_model(view->view, CarrierMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, CARRIER_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(CarrierMainView* view, CarrierToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        CarrierMainViewModel * m,
        {
...
static void send_state_immediate(CarrierMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        CarrierMainViewModel * m,
...
static bool carrier_main_view_input(InputEvent* event, void* context) {
    CarrierMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            carrier_state_set_mode(m->state, (CarrierMode)m->picker_index);
                        } else {
                            carrier_state_set_fan(m->state, (CarrierFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        CarrierToggle t = (CarrierToggle)(f - SLOT_TOGGLE_0 + 1);
                        carrier_state_toggle(m->state, t);
...
CarrierMainView* carrier_main_view_alloc(void) {
    CarrierMainView* view = malloc(sizeof(CarrierMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(CarrierMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, carrier_main_view_draw);
    view_set_input_callback(view->view, carrier_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(carrier_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        CarrierMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = CarrierTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void carrier_main_view_set_state(CarrierMainView* view, CarrierState* state) {
    with_view_model(view->view, CarrierMainViewModel * m, { m->state = state; }, true);
}
...
void carrier_main_view_update_sending(CarrierMainView* view) {
    with_view_model(
        view->view, CarrierMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
