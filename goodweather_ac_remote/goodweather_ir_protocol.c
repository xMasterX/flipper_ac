...
static const char* MODE_NAMES[GoodweatherModeCount] =
...
bool goodweather_ir_encode_toggle(
    ...
    build_state(
        req,
        toggle != GoodweatherTogglePowerOff,
        swing_for(req),
        TOGGLE_CMDS[toggle],
        st);
    return encode_state_bytes(st, timings, timings_count);
...
void goodweather_ir_format_toggle(
    ...
    uint8_t st[STATE_LEN];
    build_state(
        req, toggle != GoodweatherTogglePowerOff, swing_for(req), TOGGLE_CMDS[toggle], st);
