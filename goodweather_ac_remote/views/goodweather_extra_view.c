#include "goodweather_extra_view.h"
...
} GoodweatherExtraViewModel;

struct GoodweatherExtraView {
    View* view;
    GoodweatherExtraViewSendCallback send_callback;
    void* send_context;
};

static void goodweather_extra_view_draw(Canvas* canvas, void* model) {
    GoodweatherExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)GoodweatherExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, goodweather_ir_get_extra_name((GoodweatherExtra)i));
    ...
    char buf[GOODWEATHER_CODE_STR_LEN];
    ...
    GoodweatherRequest req = goodweather_state_request(m->state);
    goodweather_ir_format_extra(&req, (GoodweatherExtra)m->selected, buf, sizeof(buf));
...
static bool goodweather_extra_view_input(InputEvent* event, void* context) {
    GoodweatherExtraView* view = context;
    bool consumed = false;
    bool send = false;
    GoodweatherExtra to_send = (GoodweatherExtra)0;

    with_view_model(
        view->view,
        GoodweatherExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)GoodweatherExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (GoodweatherExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
GoodweatherExtraView* goodweather_extra_view_alloc(void) {
    GoodweatherExtraView* view = malloc(sizeof(GoodweatherExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GoodweatherExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, goodweather_extra_view_draw);
    view_set_input_callback(view->view, goodweather_extra_view_input);
...
void goodweather_extra_view_set_state(GoodweatherExtraView* view, GoodweatherState* state) {
    with_view_model(view->view, GoodweatherExtraViewModel * m, { m->state = state; }, true);
}

void goodweather_extra_view_set_last_sent(GoodweatherExtraView* view, const char* last_sent) {
    with_view_model(view->view, GoodweatherExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
