#include "goodweather_main_view.h"
...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every GoodweatherToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from GoodweatherToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)GoodweatherToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= GOODWEATHER_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to GoodweatherExtra");
...
typedef struct {
    ...
    GoodweatherToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} GoodweatherMainViewModel;

struct GoodweatherMainView {
    View* view;
    GoodweatherMainViewSendCallback send_callback;
    void* send_context;
    GoodweatherMainViewNavigateCallback extra_callback;
    void* extra_context;
    GoodweatherMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void goodweather_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = goodweather_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? GoodweatherModeCount : GoodweatherFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? goodweather_ir_get_mode_name((GoodweatherMode)i) :
                             goodweather_ir_get_fan_name((GoodweatherFan)i);
}

static void draw_picker(Canvas* canvas, GoodweatherMainViewModel* m) {
...
static void goodweather_main_view_draw(Canvas* canvas, void* model) {
    GoodweatherMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = goodweather_ir_get_fan_name(m->state->fan);
    if(m->state->mode == GoodweatherModeOff) {
        fan_value = "--";
    } else if(goodweather_ir_mode_locks_fan(m->state->mode)) {
        fan_value = goodweather_ir_get_fan_name(GoodweatherFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", goodweather_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!goodweather_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            GoodweatherToggle t = (GoodweatherToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, goodweather_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            GoodweatherToggle t = (GoodweatherToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, goodweather_ir_get_toggle_name(t), focused,
                goodweather_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(GoodweatherMainView* view) {
    with_view_model(view->view, GoodweatherMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, GOODWEATHER_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(GoodweatherMainView* view, GoodweatherToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        GoodweatherMainViewModel * m,
        {
...
static void send_state_immediate(GoodweatherMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        GoodweatherMainViewModel * m,
...
static bool goodweather_main_view_input(InputEvent* event, void* context) {
    GoodweatherMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            goodweather_state_set_mode(m->state, (GoodweatherMode)m->picker_index);
                        } else {
                            goodweather_state_set_fan(m->state, (GoodweatherFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        GoodweatherToggle t = (GoodweatherToggle)(f - SLOT_TOGGLE_0 + 1);
                        goodweather_state_toggle(m->state, t);
...
GoodweatherMainView* goodweather_main_view_alloc(void) {
    GoodweatherMainView* view = malloc(sizeof(GoodweatherMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(GoodweatherMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, goodweather_main_view_draw);
    view_set_input_callback(view->view, goodweather_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(goodweather_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        GoodweatherMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = GoodweatherTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void goodweather_main_view_set_state(GoodweatherMainView* view, GoodweatherState* state) {
    with_view_model(view->view, GoodweatherMainViewModel * m, { m->state = state; }, true);
}
...
void goodweather_main_view_update_sending(GoodweatherMainView* view) {
    with_view_model(
        view->view, GoodweatherMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
...
int goodweather_main_view_get_last_command(GoodweatherMainView* view, GoodweatherToggle* out_toggle) {
    int result = 0;
    with_view_model(
        view->view,
        GoodweatherMainViewModel * m,
