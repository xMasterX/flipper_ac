...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every DelonghiToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from DelonghiToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)DelonghiToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= DELONGHI_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to DelonghiExtra");
...
typedef struct {
    ...
    DelonghiToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} DelonghiMainViewModel;

struct DelonghiMainView {
    View* view;
    DelonghiMainViewSendCallback send_callback;
    void* send_context;
    DelonghiMainViewNavigateCallback extra_callback;
    void* extra_context;
    DelonghiMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void delonghi_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = delonghi_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? DelonghiModeCount : DelonghiFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? delonghi_ir_get_mode_name((DelonghiMode)i) :
                             delonghi_ir_get_fan_name((DelonghiFan)i);
}

static void draw_picker(Canvas* canvas, DelonghiMainViewModel* m) {
...
static void delonghi_main_view_draw(Canvas* canvas, void* model) {
    DelonghiMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = delonghi_ir_get_fan_name(m->state->fan);
    if(m->state->mode == DelonghiModeOff) {
        fan_value = "--";
    } else if(delonghi_ir_mode_locks_fan(m->state->mode)) {
        fan_value = delonghi_ir_get_fan_name(DelonghiFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", delonghi_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!delonghi_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            DelonghiToggle t = (DelonghiToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, delonghi_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            DelonghiToggle t = (DelonghiToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, delonghi_ir_get_toggle_name(t), focused,
                delonghi_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(DelonghiMainView* view) {
    with_view_model(view->view, DelonghiMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, DELONGHI_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(DelonghiMainView* view, DelonghiToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        DelonghiMainViewModel * m,
        {
...
static void send_state_immediate(DelonghiMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        DelonghiMainViewModel * m,
...
static bool delonghi_main_view_input(InputEvent* event, void* context) {
    DelonghiMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            delonghi_state_set_mode(m->state, (DelonghiMode)m->picker_index);
                        } else {
                            delonghi_state_set_fan(m->state, (DelonghiFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        DelonghiToggle t = (DelonghiToggle)(f - SLOT_TOGGLE_0 + 1);
                        delonghi_state_toggle(m->state, t);
...
DelonghiMainView* delonghi_main_view_alloc(void) {
    DelonghiMainView* view = malloc(sizeof(DelonghiMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(DelonghiMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, delonghi_main_view_draw);
    view_set_input_callback(view->view, delonghi_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(delonghi_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        DelonghiMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = DelonghiTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void delonghi_main_view_set_state(DelonghiMainView* view, DelonghiState* state) {
    with_view_model(view->view, DelonghiMainViewModel * m, { m->state = state; }, true);
}
...
void delonghi_main_view_update_sending(DelonghiMainView* view) {
    with_view_model(
        view->view, DelonghiMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
