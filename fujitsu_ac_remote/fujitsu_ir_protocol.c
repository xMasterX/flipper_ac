...
static void build_long(const FujitsuRequest* req, uint8_t* st) {
    ...
    if(is_short_family(model)) {
        ...
        uint8_t sum = 0;
        for(uint8_t i = 0; i < llen - 1; i++) sum += st[i];
        ...
        uint8_t sum = 0;
        for(uint8_t i = slen; i < llen - 1; i++) sum += st[i];
...
bool fujitsu_ir_encode_toggle(
    ...
    uint8_t cmd;
    switch(toggle) {
    case FujitsuTogglePowerOff: cmd = F_CMD_TURN_OFF; break;
    case FujitsuTogglePowerful: cmd = F_CMD_POWERFUL; break;
    case FujitsuToggleEcono: cmd = F_CMD_ECONO; break;
    default: {
