...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every FujitsuToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from FujitsuToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)FujitsuToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= FUJITSU_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to FujitsuExtra");
...
typedef struct {
    ...
    FujitsuToggle last_toggle;

    bool state_pending;

    PickerKind picker_kind;
    uint8_t picker_index;
} FujitsuMainViewModel;

struct FujitsuMainView {
    View* view;
    FujitsuMainViewSendCallback send_callback;
    void* send_context;
    FujitsuMainViewNavigateCallback extra_callback;
    void* extra_context;
    FujitsuMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void fujitsu_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = fujitsu_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? FujitsuModeCount : FujitsuFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? fujitsu_ir_get_mode_name((FujitsuMode)i) :
                             fujitsu_ir_get_fan_name((FujitsuFan)i);
}

static void draw_picker(Canvas* canvas, FujitsuMainViewModel* m) {
...
static void fujitsu_main_view_draw(Canvas* canvas, void* model) {
    FujitsuMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = fujitsu_ir_get_fan_name(m->state->fan);
    if(m->state->mode == FujitsuModeOff) {
        fan_value = "--";
    } else if(fujitsu_ir_mode_locks_fan(m->state->mode)) {
        fan_value = fujitsu_ir_get_fan_name(FujitsuFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", fujitsu_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!fujitsu_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            FujitsuToggle t = (FujitsuToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, fujitsu_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            FujitsuToggle t = (FujitsuToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, fujitsu_ir_get_toggle_name(t), focused,
                fujitsu_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(FujitsuMainView* view) {
    with_view_model(view->view, FujitsuMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, FUJITSU_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(FujitsuMainView* view, FujitsuToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        FujitsuMainViewModel * m,
        {
...
static void send_state_immediate(FujitsuMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        FujitsuMainViewModel * m,
...
static bool fujitsu_main_view_input(InputEvent* event, void* context) {
    FujitsuMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            fujitsu_state_set_mode(m->state, (FujitsuMode)m->picker_index);
                        } else {
                            fujitsu_state_set_fan(m->state, (FujitsuFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        FujitsuToggle t = (FujitsuToggle)(f - SLOT_TOGGLE_0 + 1);
                        fujitsu_state_toggle(m->state, t);
...
FujitsuMainView* fujitsu_main_view_alloc(void) {
    FujitsuMainView* view = malloc(sizeof(FujitsuMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(FujitsuMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, fujitsu_main_view_draw);
    view_set_input_callback(view->view, fujitsu_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(fujitsu_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        FujitsuMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = FujitsuTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void fujitsu_main_view_set_state(FujitsuMainView* view, FujitsuState* state) {
    with_view_model(view->view, FujitsuMainViewModel * m, { m->state = state; }, true);
}
...
void fujitsu_main_view_update_sending(FujitsuMainView* view) {
    with_view_model(
        view->view, FujitsuMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
