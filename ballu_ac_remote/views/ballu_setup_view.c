...
struct BalluSetupView {
    View* view;
};

static uint8_t row_count(void) {
    return ballu_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void ballu_setup_view_draw(Canvas* canvas, void* model) {
    BalluSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = ballu_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, ballu_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            ballu_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)ballu_ir_get_option_count());
...
static bool ballu_setup_view_input(InputEvent* event, void* context) {
    BalluSetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        BalluSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = ballu_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
BalluSetupView* ballu_setup_view_alloc(void) {
    BalluSetupView* view = malloc(sizeof(BalluSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(BalluSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, ballu_setup_view_draw);
    view_set_input_callback(view->view, ballu_setup_view_input);
...
void ballu_setup_view_set_state(BalluSetupView* view, BalluState* state) {
    with_view_model(view->view, BalluSetupViewModel * m, { m->state = state; }, true);
}
