...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every BalluToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from BalluToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)BalluToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= BALLU_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to BalluExtra");
...
struct BalluMainView {
    View* view;
    BalluMainViewSendCallback send_callback;
    void* send_context;
    BalluMainViewNavigateCallback extra_callback;
    void* extra_context;
    BalluMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void ballu_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = ballu_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? BalluModeCount : BalluFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? ballu_ir_get_mode_name((BalluMode)i) :
                             ballu_ir_get_fan_name((BalluFan)i);
}

static void draw_picker(Canvas* canvas, BalluMainViewModel* m) {
...
static void ballu_main_view_draw(Canvas* canvas, void* model) {
    BalluMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = ballu_ir_get_fan_name(m->state->fan);
    if(m->state->mode == BalluModeOff) {
        fan_value = "--";
    } else if(ballu_ir_mode_locks_fan(m->state->mode)) {
        fan_value = ballu_ir_get_fan_name(BalluFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", ballu_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!ballu_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            BalluToggle t = (BalluToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, ballu_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            BalluToggle t = (BalluToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, ballu_ir_get_toggle_name(t), focused,
                ballu_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(BalluMainView* view) {
    with_view_model(view->view, BalluMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, BALLU_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(BalluMainView* view, BalluToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        BalluMainViewModel * m,
        {
...
static void send_state_immediate(BalluMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        BalluMainViewModel * m,
...
static bool ballu_main_view_input(InputEvent* event, void* context) {
    BalluMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            ballu_state_set_mode(m->state, (BalluMode)m->picker_index);
                        } else {
                            ballu_state_set_fan(m->state, (BalluFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        BalluToggle t = (BalluToggle)(f - SLOT_TOGGLE_0 + 1);
                        ballu_state_toggle(m->state, t);
...
BalluMainView* ballu_main_view_alloc(void) {
    BalluMainView* view = malloc(sizeof(BalluMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(BalluMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, ballu_main_view_draw);
    view_set_input_callback(view->view, ballu_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(ballu_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        BalluMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = BalluTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void ballu_main_view_set_state(BalluMainView* view, BalluState* state) {
    with_view_model(view->view, BalluMainViewModel * m, { m->state = state; }, true);
}
...
void ballu_main_view_update_sending(BalluMainView* view) {
    with_view_model(
        view->view, BalluMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
