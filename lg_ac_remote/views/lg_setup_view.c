...
static uint8_t row_count(void) {
    return lg_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void lg_setup_view_draw(Canvas* canvas, void* model) {
    LgSetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = lg_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, lg_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            lg_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)lg_ir_get_option_count());
...
static bool lg_setup_view_input(InputEvent* event, void* context) {
    LgSetupView* view = context;
    bool consumed = false;
    ...
    with_view_model(
        ...
        LgSetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                uint8_t opts = lg_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
LgSetupView* lg_setup_view_alloc(void) {
    LgSetupView* view = malloc(sizeof(LgSetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(LgSetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, lg_setup_view_draw);
    view_set_input_callback(view->view, lg_setup_view_input);
...
void lg_setup_view_set_state(LgSetupView* view, LgState* state) {
    with_view_model(view->view, LgSetupViewModel * m, { m->state = state; }, true);
}
