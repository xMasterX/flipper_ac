...
//   0 Mode        1 Fan
//   2 Temp -      3 Temp +
//   4.. toggles (every LgToggle except PowerOff, in enum order)
//   then Extra, then Setup
// Everything below is derived from LgToggleCount, so a port never edits
...
#define TOGGLE_SLOTS ((int)LgToggleCount - 1)
#define SLOT_EXTRA   (SLOT_TOGGLE_0 + TOGGLE_SLOTS)
#define SLOT_SETUP   (SLOT_EXTRA + 1)
#define SLOT_COUNT   (SLOT_SETUP + 1)

_Static_assert(
    TOGGLE_SLOTS <= LG_MAX_MAIN_TOGGLES,
    "Too many main-screen toggles - move some to LgExtra");
...
struct LgMainView {
    View* view;
    LgMainViewSendCallback send_callback;
    void* send_context;
    LgMainViewNavigateCallback extra_callback;
    void* extra_context;
    LgMainViewNavigateCallback setup_callback;
    void* setup_context;
    FuriTimer* debounce_timer;
};

static void lg_main_view_debounce_callback(void* context);
...
static void draw_footer(Canvas* canvas, bool sending, int frame) {
    const char* name = lg_ir_get_protocol_name();
...
static uint8_t picker_count(PickerKind k) {
    return k == PickerMode ? LgModeCount : LgFanCount;
}

static const char* picker_name(PickerKind k, uint8_t i) {
    return k == PickerMode ? lg_ir_get_mode_name((LgMode)i) :
                             lg_ir_get_fan_name((LgFan)i);
}

static void draw_picker(Canvas* canvas, LgMainViewModel* m) {
...
static void lg_main_view_draw(Canvas* canvas, void* model) {
    LgMainViewModel* m = model;
    if(!m->state) return;

    canvas_clear(canvas);

    // Row 0: Mode and Fan
    const char* fan_value = lg_ir_get_fan_name(m->state->fan);
    if(m->state->mode == LgModeOff) {
        fan_value = "--";
    } else if(lg_ir_mode_locks_fan(m->state->mode)) {
        fan_value = lg_ir_get_fan_name(LgFanAuto);
    }
    draw_selector_button(
        canvas, COL_X(0), 0, BTN_W, "Mode", lg_ir_get_mode_name(m->state->mode),
        m->focus == SLOT_MODE);
    draw_selector_button(canvas, COL_X(1), 0, BTN_W, "Fan", fan_value, m->focus == SLOT_FAN);

    // Row 1: temperature
    if(!lg_state_can_change_temp(m->state)) {
        canvas_set_font(canvas, FontBigNumbers);
    ...
    if(TWO_LINE_FITS) {
        for(int slot = SLOT_TOGGLE_0; slot < SLOT_EXTRA; slot++) {
            LgToggle t = (LgToggle)(slot - SLOT_TOGGLE_0 + 1);
            if(canvas_string_width(canvas, lg_ir_get_toggle_name(t)) > BTN_W - 4) {
                wrap_needed = true;
                break;
            }
        }
    ...
    for(int slot = SLOT_TOGGLE_0; slot < SLOT_COUNT; slot++) {
        ...
        if(slot == SLOT_EXTRA) {
            ...
            LgToggle t = (LgToggle)(slot - SLOT_TOGGLE_0 + 1);
            draw_button(
                canvas, x, y, btn_h, lg_ir_get_toggle_name(t), focused,
                lg_state_toggle_active(m->state, t));
        }
    }
...
static void schedule_state_send(LgMainView* view) {
    with_view_model(view->view, LgMainViewModel * m, { m->state_pending = true; }, false);
    furi_timer_stop(view->debounce_timer);
    furi_timer_start(view->debounce_timer, LG_SEND_DEBOUNCE_MS);
}

static void send_toggle_immediate(LgMainView* view, LgToggle toggle) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        LgMainViewModel * m,
...
static void send_state_immediate(LgMainView* view) {
    furi_timer_stop(view->debounce_timer);
    with_view_model(
        view->view,
        LgMainViewModel * m,
...
static bool lg_main_view_input(InputEvent* event, void* context) {
    LgMainView* view = context;
    bool consumed = false;
            ...
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        if(m->picker_kind == PickerMode) {
                            lg_state_set_mode(m->state, (LgMode)m->picker_index);
                        } else {
                            lg_state_set_fan(m->state, (LgFan)m->picker_index);
                        }
                ...
                case InputKeyOk: {
                    ...
                    if(f == SLOT_MODE) {
                        ...
                        LgToggle t = (LgToggle)(f - SLOT_TOGGLE_0 + 1);
                        lg_state_toggle(m->state, t);
...
LgMainView* lg_main_view_alloc(void) {
    LgMainView* view = malloc(sizeof(LgMainView));
    ...
    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(LgMainViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, lg_main_view_draw);
    view_set_input_callback(view->view, lg_main_view_input);
    view_set_orientation(view->view, ViewOrientationVertical);

    view->debounce_timer =
        furi_timer_alloc(lg_main_view_debounce_callback, FuriTimerTypeOnce, view);
    ...
    with_view_model(
        ...
        LgMainViewModel * m,
        {
            ...
            m->send_anim_frame = 0;
            m->last_was_toggle = false;
            m->last_toggle = LgTogglePowerOff;
            m->state_pending = false;
            m->picker_kind = PickerNone;
            m->picker_index = 0;
        },
...
void lg_main_view_set_state(LgMainView* view, LgState* state) {
    with_view_model(view->view, LgMainViewModel * m, { m->state = state; }, true);
}
...
void lg_main_view_update_sending(LgMainView* view) {
    with_view_model(
        view->view, LgMainViewModel * m, { if(m->is_sending) m->send_anim_frame++; }, true);
}
