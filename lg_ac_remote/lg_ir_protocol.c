...
static const char* EXTRA_NAMES[LgExtraCount] = {
    "Vane 1", "Vane 2", "Vane 3", "Vane 4", "Vane 5", "Vane 6",
    "SwingH on", "SwingH off"};
...
bool lg_ir_encode_state(const LgRequest* req, uint32_t* timings, size_t* timings_count) {
    ...
    return encode_word(build_state(req->mode, req->fan, req->temp), req->option, timings, timings_count);
}
