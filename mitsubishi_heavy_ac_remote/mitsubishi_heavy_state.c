...
bool mitsubishi_heavy_state_load(MitsubishiHeavyState* state) {
    ...
    if(storage_file_open(file, MITSUBISHI_HEAVY_STATE_FILE_PATH, FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        if(storage_file_read(file, &magic, sizeof(magic)) == sizeof(magic) &&
           ...
           magic == MITSUBISHI_HEAVY_STATE_MAGIC && version == MITSUBISHI_HEAVY_STATE_FILE_VERSION) {
            MitsubishiHeavyStateStorage tmp;
            if(storage_file_read(file, &tmp, sizeof(tmp)) == sizeof(tmp)) {
                if(tmp.mode < MitsubishiHeavyModeCount && tmp.last_active_mode < MitsubishiHeavyModeCount &&
                   tmp.last_active_mode != MitsubishiHeavyModeOff && tmp.fan < MitsubishiHeavyFanCount &&
                   tmp.temp >= MITSUBISHI_HEAVY_TEMP_MIN && tmp.temp <= MITSUBISHI_HEAVY_TEMP_MAX &&
...
bool mitsubishi_heavy_state_toggle_active(const MitsubishiHeavyState* state, MitsubishiHeavyToggle toggle) {
...
MitsubishiHeavyRequest mitsubishi_heavy_state_request(const MitsubishiHeavyState* state) {
    MitsubishiHeavyRequest req = {MitsubishiHeavyModeOff, MitsubishiHeavyFanAuto, MITSUBISHI_HEAVY_DEFAULT_TEMP, 0, 0};
    if(!state) return req;
    req.mode = state->mode;
    req.fan = state->fan;
    req.temp = state->temp;
...
void mitsubishi_heavy_state_format_current(const MitsubishiHeavyState* state, char* out, size_t len) {
