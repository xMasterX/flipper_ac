#include "mitsubishi_heavy_setup_view.h"
...
typedef struct {
    ...
    uint8_t focus;
} MitsubishiHeavySetupViewModel;

struct MitsubishiHeavySetupView {
    View* view;
};

static uint8_t row_count(void) {
    return mitsubishi_heavy_ir_get_option_count() > 0 ? 2 : 1;
}
...
static void mitsubishi_heavy_setup_view_draw(Canvas* canvas, void* model) {
    MitsubishiHeavySetupViewModel* m = model;
    if(!m->state) return;
    ...
    bool has_opt = mitsubishi_heavy_ir_get_option_count() > 0;
    ...
    if(has_opt) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str_aligned(
            canvas, 32, oy + 3, AlignCenter, AlignTop, mitsubishi_heavy_ir_get_option_label());
        ...
        canvas_draw_str_aligned(
            ...
            AlignCenter,
            AlignTop,
            mitsubishi_heavy_ir_get_option_name(m->state->option));
        ...
        snprintf(
            ...
            (unsigned)mitsubishi_heavy_ir_get_option_count());
...
static bool mitsubishi_heavy_setup_view_input(InputEvent* event, void* context) {
    MitsubishiHeavySetupView* view = context;
    bool consumed = false;

    with_view_model(
        view->view,
        MitsubishiHeavySetupViewModel * m,
        {
            if(event->type != InputTypeShort && event->type != InputTypeRepeat) {
                // nothing
            } else {
                uint8_t rows = row_count();
                uint8_t opts = mitsubishi_heavy_ir_get_option_count();
                switch(event->key) {
                case InputKeyUp:
...
MitsubishiHeavySetupView* mitsubishi_heavy_setup_view_alloc(void) {
    MitsubishiHeavySetupView* view = malloc(sizeof(MitsubishiHeavySetupView));
    view->view = view_alloc();

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MitsubishiHeavySetupViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, mitsubishi_heavy_setup_view_draw);
    view_set_input_callback(view->view, mitsubishi_heavy_setup_view_input);
...
void mitsubishi_heavy_setup_view_set_state(MitsubishiHeavySetupView* view, MitsubishiHeavyState* state) {
    with_view_model(view->view, MitsubishiHeavySetupViewModel * m, { m->state = state; }, true);
}
