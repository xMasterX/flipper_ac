#include "mitsubishi_heavy_extra_view.h"
...
} MitsubishiHeavyExtraViewModel;

struct MitsubishiHeavyExtraView {
    View* view;
    MitsubishiHeavyExtraViewSendCallback send_callback;
    void* send_context;
};

static void mitsubishi_heavy_extra_view_draw(Canvas* canvas, void* model) {
    MitsubishiHeavyExtraViewModel* m = model;
    if(!m->state) return;
    ...
    uint8_t total = (uint8_t)MitsubishiHeavyExtraCount;
    ...
    for(uint8_t r = 0; r < shown; r++) {
        ...
        canvas_draw_str_aligned(
            canvas, 32, y + 2, AlignCenter, AlignTop, mitsubishi_heavy_ir_get_extra_name((MitsubishiHeavyExtra)i));
    ...
    char buf[MITSUBISHI_HEAVY_CODE_STR_LEN];
    ...
    MitsubishiHeavyRequest req = mitsubishi_heavy_state_request(m->state);
    mitsubishi_heavy_ir_format_extra(&req, (MitsubishiHeavyExtra)m->selected, buf, sizeof(buf));
...
static bool mitsubishi_heavy_extra_view_input(InputEvent* event, void* context) {
    MitsubishiHeavyExtraView* view = context;
    bool consumed = false;
    bool send = false;
    MitsubishiHeavyExtra to_send = (MitsubishiHeavyExtra)0;

    with_view_model(
        view->view,
        MitsubishiHeavyExtraViewModel * m,
        {
            if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
                ...
                case InputKeyUp:
                    ...
                    break;
                case InputKeyDown:
                    if(m->selected + 1 < (uint8_t)MitsubishiHeavyExtraCount) m->selected++;
                ...
                case InputKeyOk:
                    if(event->type == InputTypeShort) {
                        to_send = (MitsubishiHeavyExtra)m->selected;
                        send = true;
                    }
                    consumed = true;
                    break;
...
MitsubishiHeavyExtraView* mitsubishi_heavy_extra_view_alloc(void) {
    MitsubishiHeavyExtraView* view = malloc(sizeof(MitsubishiHeavyExtraView));
    view->view = view_alloc();
    view->send_callback = NULL;
    view->send_context = NULL;

    view_allocate_model(view->view, ViewModelTypeLocking, sizeof(MitsubishiHeavyExtraViewModel));
    view_set_context(view->view, view);
    view_set_draw_callback(view->view, mitsubishi_heavy_extra_view_draw);
    view_set_input_callback(view->view, mitsubishi_heavy_extra_view_input);
...
void mitsubishi_heavy_extra_view_set_state(MitsubishiHeavyExtraView* view, MitsubishiHeavyState* state) {
    with_view_model(view->view, MitsubishiHeavyExtraViewModel * m, { m->state = state; }, true);
}

void mitsubishi_heavy_extra_view_set_last_sent(MitsubishiHeavyExtraView* view, const char* last_sent) {
    with_view_model(view->view, MitsubishiHeavyExtraViewModel * m, { m->last_sent = last_sent; }, true);
}
