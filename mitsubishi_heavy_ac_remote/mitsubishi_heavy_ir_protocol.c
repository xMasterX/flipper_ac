...
static void build_state(
    ...
    MitsubishiHeavyFan fan =
        req->fan >= MitsubishiHeavyFanCount ? MitsubishiHeavyFanAuto : req->fan;
...
static uint8_t swing_v_for(const MitsubishiHeavyRequest* req) {
    return ((req->toggle_bits >> MitsubishiHeavyToggleSwing) & 1) ? MH_SWINGV_AUTO :
                                                                    MH_SWINGV_OFF;
}
...
bool mitsubishi_heavy_ir_encode_state(
    ...
    return encode_for_model(
        req, true, swing_v_for(req), MH_SWINGH_AUTO, timings, timings_count);
}
...
bool mitsubishi_heavy_ir_encode_extra(
    ...
    if(extra <= MitsubishiHeavyExtraVaneLowest) {
        return encode_for_model(
            req, true, VANE[extra], MH_SWINGH_AUTO, timings, timings_count);
    }
...
void mitsubishi_heavy_ir_format_state(
    const MitsubishiHeavyRequest* req,
    char* out,
    size_t len) {
