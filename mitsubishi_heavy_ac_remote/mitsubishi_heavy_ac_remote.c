...
static void mitsubishi_heavy_app_send_toggle_ir(MitsubishiHeavyApp* app, MitsubishiHeavyToggle toggle);
...
static bool mitsubishi_heavy_app_back_callback(void* context) {
    ...
    if(app->current_view == MitsubishiHeavyViewExtra || app->current_view == MitsubishiHeavyViewSetup) {
...
static MitsubishiHeavyApp* mitsubishi_heavy_app_alloc(void) {
    ...
    view_dispatcher_set_navigation_event_callback(app->view_dispatcher, mitsubishi_heavy_app_back_callback);
    ...
    mitsubishi_heavy_main_view_set_send_callback(app->main_view, mitsubishi_heavy_app_on_send, app);
    mitsubishi_heavy_main_view_set_extra_callback(app->main_view, mitsubishi_heavy_app_on_extra_navigate, app);
    mitsubishi_heavy_main_view_set_setup_callback(app->main_view, mitsubishi_heavy_app_on_setup_navigate, app);
    view_dispatcher_add_view(
        app->view_dispatcher, MitsubishiHeavyViewMain, mitsubishi_heavy_main_view_get_view(app->main_view));
    ...
    mitsubishi_heavy_extra_view_set_send_callback(app->extra_view, mitsubishi_heavy_app_on_extra_send, app);
    view_dispatcher_add_view(
        app->view_dispatcher, MitsubishiHeavyViewExtra, mitsubishi_heavy_extra_view_get_view(app->extra_view));

    // Setup view
    app->setup_view = mitsubishi_heavy_setup_view_alloc();
    mitsubishi_heavy_setup_view_set_state(app->setup_view, app->state);
    view_dispatcher_add_view(
        app->view_dispatcher, MitsubishiHeavyViewSetup, mitsubishi_heavy_setup_view_get_view(app->setup_view));
    ...
    app->ir_tx_stop_timer = furi_timer_alloc(mitsubishi_heavy_app_tx_stop_callback, FuriTimerTypeOnce, app);
    app->is_sending = false;
